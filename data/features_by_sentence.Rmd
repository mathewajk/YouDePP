...
df.observed.headVent.spoken <- df.all.avg.lm.spoken %>%
  mutate(entropyCat = case_when(entropy_scaled <= (ent_mean.spoken - ent_sd.spoken) ~ "Min entropy",
                                entropy_scaled >= (ent_mean.spoken + ent_sd.spoken) ~ "Max entropy",
                                TRUE ~ "Mean entropy"),
...
df.observed.headVent.written <- df.all.avg.lm.written %>%
  mutate(entropyCat = case_when(entropy_scaled <= (ent_mean.written - ent_sd.written) ~ "Min entropy",
                                entropy_scaled >= (ent_mean.written + ent_sd.written) ~ "Max entropy",
                                TRUE ~ "Mean entropy"),
         headednessCat = case_when(headedness_scaled <= (hf_mean.written - hf_sd.written) ~ "Min headedness",
                                headedness_scaled >= (hf_mean.written + hf_sd.written) ~ "Max headedness",
...
```{r graph_headedness_entropy_diff_max}
