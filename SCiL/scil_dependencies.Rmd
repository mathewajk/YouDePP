---
title: "SCiL - Written v. Spoken dependencies"
...
gg.deps.by.baseline <- gg.deps.by.baseline + 
    ...
    scale_color_manual(values=c("#86c300", "#736ef6", "#43abea"), labels=c("Random", "Optimal", "Observed"))
...
manual.diff.g <- ggplot(df.ud.manual.lm.diff %>% 
                        group_by(language, corpus, sentence_uid) %>% 
                        summarize(difference_score=mean(difference_score), total_length=mean(total_length)), 
                      aes(y=difference_score, x=total_length, color=language, group=language))

# GAMM fits
manual.diff.g <- manual.diff.g + 
  geom_smooth(aes(group=language), se = TRUE, method = "gam", formula = y ~ s(log(x)))
...
## Automatic v Manual regressions
...
gg.ud.youdepp.svo <- gg.ud.youdepp.svo + 
  stat_bin_hex(data=df.ud.all.observed %>% filter(language != "Japanese" & language != "Korean" & language != "Turkish"), 
               bins=50, binwidth = c(1, 10), show.legend=FALSE, inherit.aes=FALSE, 
               aes(y=dep_length, x=total_length, alpha=log(..density..)), fill="#c6d6d6")
...
ggsave("../data/plots/ud_youdepp_comparison_1.png", plot=gg.ud.youdepp.svo, device="png", dpi = 300, height=9, width=8)
gg.ud.youdepp.svo
...
ggsave("../data/plots/ud_youdepp_comparison_2.png", plot=gg.ud.youdepp.sov, device="png", dpi = 300, height=7.25, width=8)
gg.ud.youdepp.sov
```
