...
header-includes:
  ...
  - \usepackage[backend=biber, authordate, natbib, giveninits=true]{apa}
...
df.all.jp <- df.all.jp %>% mutate(ri = as.integer(baseline == "Random"), mi = as.integer(baseline == "Optimal")) %>% filter(dep_length > 5 & dep_length < 200)
...
df.ThisIsHorosho <- bind_rows(list("Observed"=df.ThisIsHorosho.observed, "Optimal"=df.ThisIsHorosho.optimal, 
                            "Random"=df.ThisIsHorosho),  .id = 'baseline')
```
...
df.all.ru <- bind_rows(list("AdamThomasMoran"=df.adam), .id = 'channel')
...
```{r avg_all_jp, include=TRUE, echo=FALSE, fig.cap="Average total dependency lengths for sentences with total length > 5 and < 200", fig.align='left', fig.show='hold'}
...
```{r avg_channel_jp, include=TRUE, echo=FALSE, fig.cap="Average total Dependency ", fig.align='left', fig.show='hold'}
# Plot by channel
...
```{r ru_all, include=TRUE, echo=FALSE, fig.cap="Average total dependency lengths for sentences with total length > 5 and < 200", fig.align='left', fig.show='hold'}
...
The results of this preliminary work are promising: it is relatively easy to acquire the subtitles for a large number of videos, and the subtitles are overall of good quality. Issues arise primarily in the preprocessing and parsing of the subtitle, especially for Japanese. 

A straightforward way to begin improving the corpus would be to diversify the channels included in the Japanese sample and to balance out the Japanese and Russian data.
