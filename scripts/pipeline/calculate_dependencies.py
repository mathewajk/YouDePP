    ...
    with open(observed_fn, "w") as observed_out, \
        ...
        for dependency_fn in dependency_fns:
            ...
            with open(dependency_fn, "r") as dependencies_in:
                ...
                process_dependencies(video_id, stanza.Document(json_data), observed_out, optimal_out, random_out)
...
def get_true_dependencies(dependencies, video_id, sent_id):
    true_dependencies = []
    for dependency in dependencies:
        if dependency[1] not in ["punct", "PUNCT"]:
            if(dependency[0].text in ["root", "ROOT"]):
                if(dependency[2].upos in ["punct", "PUNCT"]):
                    logging.warning("{0}:{1} ROOT is PUNCT".format(video_id, sent_id))
                    return([])
...
def process_dependencies(video_id, doc, observed_out, optimal_out, random_out):
    ...
    for sentence in doc.sentences:
        ...
        true_dependencies = get_true_dependencies(sentence.dependencies, video_id, sent_id)
