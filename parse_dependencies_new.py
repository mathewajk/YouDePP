...
def process_files(nlp, channel, language, type, subtitles_fns):
    ...
    for subtitles_fn in subtitles_fns:
        ...
        video_id = path.split(subtitles_fn)[1].split('_')[1]

        dependencies_fn = path.join(dep_path, channel + "_" + video_id + "_dependencies.json")
        ...
        with open(subtitles_fn, "r") as subtitles_in, open(dependencies_fn, "w") as dependencies_out:
            ...
            try:
                ...
                dependencies_json = dumps(nlp_subtitles.to_dict())
                dependencies_out.write(dependencies_json)
                break
